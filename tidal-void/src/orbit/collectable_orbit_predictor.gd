class_name CollecOrbitPredictor
extends Node
...
func _physics_process(_delta: float) -> void:
	if not collectable:
		line.queue_free()
		queue_free()
	elif collectable.b_simulate_gravity:
		if(collectable.on_screen_notifier.is_on_screen()):
			line.visible = true
			draw_orbit()
		else:
			line.visible = false
...
func draw_ellipse(focus: Vector2, 
	...
	var player_local = (collectable.global_position - center).rotated(-periapsis_angle)
	var start_angle = atan2(player_local.y / b, player_local.x / a)

	var direction = 1 if h > 0 else -1

	var collision_radius_sq = collectable.dominant_body.collision_radius ** 2
